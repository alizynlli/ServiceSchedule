...
{
    public partial class ServiceOperationDocumentReportForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        public ServiceOperationDocumentReportForm()
        {
            InitializeComponent();
        ...
        public async Task LoadReport()
        {
            ...
            try
            {
                ...
                var result = await _service.GetDocumentReport(FirstDate, LastDate, _selectedClients, _selectedServices);
                if (result.IsFailed)
                {
                ...
                gridControl1.DataSource = result.Data.ToList();
            }
        ...
        {
            return SplashScreenManager.ShowOverlayForm(gridControl1);
        }
        ...
        private void ServiceOperationDocumentReportForm_Load(object sender, EventArgs e)
        {
        ...
        private void btnExportToExcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (!(gridView1.DataSource is List<ClientServiceDocumentReportModel> lines) || lines.Count == 0)
                {
                ...
                {
                    column = 1;
                    ws.Range[row, column++].Text = line.Series;
                    ws.Range[row, column++].Text = line.Number.ToString();
